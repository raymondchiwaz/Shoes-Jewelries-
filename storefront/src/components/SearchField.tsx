...
export const SearchField: React.FC<{
  ...
  const list = useAsyncList<ListItem>({
    ...
    load: async ({ filterText, signal }) => {
      // Search is optional + region can be unavailable during bootstrap.
      if (!filterText?.trim() || !searchClient || !region) {
      ...
      try {
        ...
        // e.g. MeiliSearch not running / misconfigured. Fail silently in UI.
        return { items: [], filterText }
      }
    },
