/**
 * Stage local images to backend/static folder and generate import CSV
 * 
 * This script:
 * 1. Copies images from storefront/public/images/imports to backend/static/products
 * 2. Generates a CSV with URLs pointing to the backend's static file server
 */

const fs = require('fs');
const path = require('path');

// Backend URL where static files are served
const BACKEND_URL = process.env.BACKEND_URL || 'http://localhost:9000';
const STATIC_URL = `${BACKEND_URL}/static/products`;

// Paths
const sourceDir = path.join(__dirname, '..', 'storefront', 'public', 'images', 'imports');
const destDir = path.join(__dirname, '..', 'backend', 'static', 'products');
...
// Copy images and return their new URLs
function stageImagesForProduct(handle) {
  ...
  const destFolderPath = path.join(destDir, handle);
  
  if (!fs.existsSync(sourceFolderPath)) {
    console.warn(`Source folder not found: ${sourceFolderPath}`);
    return { thumbnail: '', images: [] };
  }
  
  // Ensure product destination folder exists
  ...
  if (files.length === 0) {
    console.warn(`No images found in: ${sourceFolderPath}`);
    return { thumbnail: '', images: [] };
  }
  
  // Copy files and build URLs
  const imageUrls = [];
  
  for (let i = 0; i < files.length; i++) {
    const file = files[i];
    const ext = path.extname(file);
    // Rename to simpler format: 1.jpg, 2.jpg, etc.
    const newFileName = `${i + 1}${ext}`;
    
    const srcPath = path.join(sourceFolderPath, file);
    const dstPath = path.join(destFolderPath, newFileName);
    
    // Copy file
    fs.copyFileSync(srcPath, dstPath);
    
    // Build URL
    imageUrls.push(`${STATIC_URL}/${handle}/${newFileName}`);
  }
...
function generateCSV() {
  ...
  for (const product of products) {
    ...
    const { thumbnail, images } = stageImagesForProduct(product.handle);
    ...
    const row = [
      ...
      'published',
      thumbnail,
      '', '', '', '', '', '', '', '', '', '', '', // Weight through Tag 1
      'TRUE', // Product Discountable
      '', // Product External Id
      '', // Variant Id
      `${product.title} - One Size`,
      product.sku,
      '', // Variant Barcode
...
console.log('Staging images and generating import CSV');
console.log('='.repeat(60));
...
const outputPath = path.join(__dirname, '..', 'backend', 'data', 'product-import-staged.csv');
...
console.log('2. Import the CSV: npx medusa product import -f data/product-import-staged.csv');
console.log('');
